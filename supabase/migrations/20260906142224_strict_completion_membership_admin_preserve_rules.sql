-- Strict completion: preserve custom membership/service mappings when editing an existing tier.
begin;
...
begin
 ...
 if not v_exists then insert into public.subscription_tier_service_rules(tier_id,feature_key,included,purchase_separately,temporary,enabled) select trim(p_tier_id),c.feature_key,true,false,true,true from public.profile_service_catalog c where c.is_active=true and c.sort_order<=p_unlocked_service_count; end if;
