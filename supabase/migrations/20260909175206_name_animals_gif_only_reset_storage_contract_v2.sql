...
SET public = true, file_size_limit = 8 * 1024 * 1024, allowed_mime_types = ARRAY['image/gif']::text[]
WHERE id = 'name-animations';
DROP POLICY IF EXISTS name_animations_owner_insert ON storage.objects;
CREATE POLICY name_animations_owner_insert ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'name-animations' AND (storage.foldername(name))[1] = 'catalog' AND lower(storage.extension(name)) = 'gif' AND private.is_platform_owner_storage());
DROP POLICY IF EXISTS name_animations_owner_update ON storage.objects;
CREATE POLICY name_animations_owner_update ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'name-animations' AND private.is_platform_owner_storage())
WITH CHECK (bucket_id = 'name-animations' AND private.is_platform_owner_storage() AND lower(storage.extension(name)) = 'gif');
...
DECLARE v_uid uuid := auth.uid(); v_key text := lower(trim(coalesce(p_effect_key,''))); v_type text := lower(trim(coalesce(p_asset_type,''))); v_sort integer;
BEGIN
  ...
  IF p_storage_path IS NULL OR p_storage_path !~ '^catalog/[A-Za-z0-9_-]+\\.gif$' THEN RAISE EXCEPTION 'INVALID_STORAGE_PATH'; END IF;
